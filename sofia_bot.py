...
def start_admin_api():
    ...
    async def api_stats(auth=Depends(check_token)):
        ...
        try:
            ...
            ru_users = await conn.fetchval("SELECT COUNT(*) FROM users WHERE language='ru' AND onboarded=TRUE")
            en_users = await conn.fetchval("SELECT COUNT(*) FROM users WHERE language='en' AND onboarded=TRUE")
            return {"total": total, "today": today, "week": week, "banned": banned,
                    "total_msg": total_msg, "new_week": new_week, "ru_users": ru_users, "en_users": en_users}
        finally:
            await conn.close()
    ...
    async def api_history(user_id: int, auth=Depends(check_token)):
        conn = await db()
        try:
            rows = await conn.fetch("SELECT role, content, created_at FROM history WHERE user_id=$1 ORDER BY created_at DESC LIMIT 50", user_id)
