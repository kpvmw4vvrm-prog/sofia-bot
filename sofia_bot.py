...
ЦЕЛИ:
Если пользователь говорит о своих целях (похудеть, научиться, накопить, бросить, начать, достичь и т.д.) — ОБЯЗАТЕЛЬНО спроси: "Хотите я зафиксирую это как цель и буду отслеживать прогресс? 🎯"
Если пользователь говорит "да" или "да, запиши" после этого вопроса — это значит нужно записать цель. Ответь: "Записала! Цель добавлена в ваш планер 🎯" и в конце сообщения добавь специальный тег: [SAVE_GOAL: название цели]
...
async def process_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str):
    ...
    # Сбрасываем залипшие флаги если это спецкоманда или обычный разговор
    PRIORITY_FLAGS = ["waiting_shopping","waiting_note","waiting_finance","waiting_habit",
                      "waiting_goal","waiting_birthday"]
    # Всегда сбрасываем если это команда рисования, поиска, погоды, новостей
    if is_image_gen_request(user_text) or is_search_request(user_text) or is_weather_request(user_text) or is_news_request(user_text):
    ...
    try:
        ...
        # Добавляем цели пользователя в контекст Софии
        try:
            async with db_pool.acquire() as conn:
                user_goals = await conn.fetch("SELECT title, progress FROM goals WHERE user_id=$1 AND progress < 100 ORDER BY created_at DESC LIMIT 10", user_id)
                user_reminders = await conn.fetch("SELECT time_str, text FROM reminders WHERE user_id=$1 ORDER BY time_str LIMIT 10", user_id)
            goals_block = ""
            if user_goals:
                g_list = [f"• {g['title']} ({g['progress']}%)" for g in user_goals]
                goals_block = "\n\nАктивные цели пользователя:\n" + "\n".join(g_list)
            reminders_block = ""
            if user_reminders:
                r_list = [f"• {r['time_str']} — {r['text']}" for r in user_reminders]
                reminders_block = "\n\nНапоминания пользователя:\n" + "\n".join(r_list)
        except:
            goals_block = ""
            reminders_block = ""
        full_system = f"Сегодня: {date_str}\nСтиль общения: {comm_style}{memory_block}{goals_block}{reminders_block}\n\n{system_prompt}"
        ...
        # Проверяем рецепт
        # Автосохранение цели если AI решил записать её
        if "[SAVE_GOAL:" in reply:
            import re as _re
            goal_match = _re.search(r'\[SAVE_GOAL:\s*(.+?)\]', reply)
            if goal_match:
                goal_title = goal_match.group(1).strip()
                try:
                    async with db_pool.acquire() as conn:
                        await conn.execute("INSERT INTO goals (user_id, title) VALUES ($1, $2)", user_id, goal_title)
                    logging.info(f"Auto-saved goal: {goal_title} for user {user_id}")
                except Exception as ge:
                    logging.error(f"goal auto-save: {ge}")
            reply = _re.sub(r'\[SAVE_GOAL:[^\]]*\]', '', reply).strip()
