...
def get_main_menu(lang="ru"):
    ...
    keyboard = [
        [
            InlineKeyboardButton("📅 Планирование" if ru else "📅 Planning", web_app=WebAppInfo(url=f"{BASE}?tab=planning")),
            InlineKeyboardButton("🎯 Цели" if ru else "🎯 Goals", web_app=WebAppInfo(url=f"{BASE}?tab=goals")),
        ],
        [
            InlineKeyboardButton("📓 Дневник" if ru else "📓 Diary", web_app=WebAppInfo(url=f"{BASE}?tab=diary")),
            InlineKeyboardButton("❤️ Здоровье" if ru else "❤️ Health", web_app=WebAppInfo(url=f"{BASE}?tab=health")),
        ],
        [InlineKeyboardButton("⚙️ Настройки" if ru else "⚙️ Settings", web_app=WebAppInfo(url=f"{BASE}?tab=settings"))],
        [InlineKeyboardButton("✨ Возможности Софии" if ru else "✨ Sofia's Features", web_app=WebAppInfo(url=f"{BASE}?tab=about"))],
