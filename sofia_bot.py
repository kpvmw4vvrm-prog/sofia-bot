...
def get_main_menu(lang="ru"):
    ...
    keyboard = [
        [InlineKeyboardButton("📅 Планирование" if ru else "📅 Planning", web_app=WebAppInfo(url=f"https://sofia-production-fcc2.up.railway.app/planner.html?tab=planning"))],
        [InlineKeyboardButton("🎯 Цели" if ru else "🎯 Goals", web_app=WebAppInfo(url=f"https://sofia-production-fcc2.up.railway.app/planner.html?tab=goals"))],
        [InlineKeyboardButton("🗂 Организация" if ru else "🗂 Organization", web_app=WebAppInfo(url=f"https://sofia-production-fcc2.up.railway.app/planner.html?tab=org"))],
        [InlineKeyboardButton("❤️ Здоровье" if ru else "❤️ Health", web_app=WebAppInfo(url=f"https://sofia-production-fcc2.up.railway.app/planner.html?tab=health"))],
        [InlineKeyboardButton("⚙️ Настройки" if ru else "⚙️ Settings", callback_data="menu_settings")],
        [InlineKeyboardButton("✖️ Закрыть" if ru else "✖️ Close", callback_data="close_menu")],
    ]
