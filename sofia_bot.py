...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing and existing.get("onboarded"):
        ...
        name = existing.get("name") or user_name
        if lang == "ru":
    ...
    # Новый пользователь ИЛИ после сброса — начинаем онбординг заново
    await save_user(user_id, username=username, name=None)
    await update.message.reply_text(t("ru", "welcome"))
    if not existing:
        await notify_admin_event(context, f"🆕 Новый пользователь!\nИмя: {user_name}\nUsername: @{username}\nID: {user_id}")
    else:
        await notify_admin_event(context, f"🔄 Пользователь сбросил аккаунт\nUsername: @{username}\nID: {user_id}")
    return ASK_NAME
