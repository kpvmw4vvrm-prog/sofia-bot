...
# ── ПОДПИСКА ──────────────────────────────────
SUBSCRIPTION_PRICE = 490          # руб/мес
TRIAL_DAYS = 10                   # дней пробного периода
STARS_PRICE = 50                  # Telegram Stars
...
async def create_trial(user_id: int):
    """Создаём пробный период при первом входе"""
...
async def is_subscribed(user_id: int) -> bool:
    ...
    try:
        ...
        if not row:
            ...
            return True  # новый пользователь — даём доступ
        if row['is_free']:
            return True
        if row['status'] == 'trial' and row['trial_end'] and row['trial_end'] > datetime.now():
            return True
        if row['status'] == 'active' and row['paid_until'] and row['paid_until'] > datetime.now():
            return True
        return False
    finally:
        await conn.close()

async def get_sub_info(user_id: int) -> dict:
    ...
    try:
        ...
        now = datetime.now()
        if row['is_free']:
            return {"status": "free", "reason": row['free_reason'] or ""}
        if row['status'] == 'trial' and row['trial_end']:
            days = (row['trial_end'] - now).days
            return {"status": "trial", "days_left": max(0, days), "trial_end": row['trial_end']}
        if row['status'] == 'active' and row['paid_until']:
            days = (row['paid_until'] - now).days
            return {"status": "active", "days_left": max(0, days), "paid_until": row['paid_until']}
        return {"status": "expired", "days_left": 0}
    finally:
        await conn.close()

async def activate_subscription(user_id: int, months: int = 1, method: str = "card"):
    """Активируем платную подписку"""
    from datetime import datetime, timedelta
    conn = await db()
    try:
        row = await conn.fetchrow("SELECT paid_until FROM subscriptions WHERE user_id=$1", user_id)
        now = datetime.now()
        if row and row['paid_until'] and row['paid_until'] > now:
            new_until = row['paid_until'] + timedelta(days=30 * months)
        else:
            new_until = now + timedelta(days=30 * months)
        await conn.execute("""
            INSERT INTO subscriptions (user_id, status, paid_until, payment_method, total_paid, last_payment)
            VALUES ($1, 'active', $2, $3, $4, NOW())
            ON CONFLICT (user_id) DO UPDATE SET
                status='active', paid_until=$2, payment_method=$3,
                total_paid=subscriptions.total_paid+$4, last_payment=NOW()
        """, user_id, new_until, method, SUBSCRIPTION_PRICE * months)
    finally:
        await conn.close()

async def set_free_access(user_id: int, reason: str = ""):
    """Даём бесплатный доступ"""
    conn = await db()
    try:
        await conn.execute("""
            INSERT INTO subscriptions (user_id, status, is_free, free_reason)
            VALUES ($1, 'free', TRUE, $2)
            ON CONFLICT (user_id) DO UPDATE SET is_free=TRUE, free_reason=$2
        """, user_id, reason)
    finally:
        await conn.close()

async def revoke_free_access(user_id: int):
    """Убираем бесплатный доступ"""
    conn = await db()
    try:
        await conn.execute("""
            UPDATE subscriptions SET is_free=FALSE, free_reason=NULL WHERE user_id=$1
        """, user_id)
    finally:
        await conn.close()
...
def get_submenu(section, lang="ru"):
    ...
    if section == "planning":
        ...
        keyboard = [
            [InlineKeyboardButton("🛒 Покупки", web_app=WebAppInfo(url=f"{BASE}?tab=diary&view=shop")),
             InlineKeyboardButton("💰 Финансы", web_app=WebAppInfo(url=f"{BASE}?tab=diary&view=fin"))],
            [InlineKeyboardButton("🍳 Рецепты", web_app=WebAppInfo(url=f"{BASE}?tab=diary&view=recipes")),
             InlineKeyboardButton("🎂 Дни рождения", web_app=WebAppInfo(url=f"{BASE}?tab=diary&view=bdays"))],
            [back],
        ]
...
async def finish_onboarding_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await create_trial(user_id)  # Запускаем пробный период
    user = await get_user(user_id)
    lang = user.get("language","ru") if user else "ru"
    name = user["name"] if user else ""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data in ("pay_yukassa", "pay_stars", "sub_info"):
        await handle_pay_callback(query, user_id, lang, context)
...
async def handle_pay_callback(query, user_id: int, lang: str, context):
    """Обрабатываем нажатие кнопок оплаты"""
    ...
    if query.data == "pay_stars":
        ...
        await context.bot.send_invoice(
            chat_id=user_id,
            title="Подписка на Софию",
            description=f"Доступ к персональному ассистенту на 30 дней",
            payload=f"sub_stars_{user_id}",
    ...
    elif query.data == "pay_yukassa":
        if not YUKASSA_SHOP_ID:
            await query.answer("ЮKassa ещё настраивается, попробуйте позже", show_alert=True)
            return
        try:
            import httpx
            async with httpx.AsyncClient() as client:
                resp = await client.post(
                    ...
                    headers={"Idempotence-Key": f"sub_{user_id}_{int(__import__('time').time())}"},
                    json={
                        "amount": {"value": str(SUBSCRIPTION_PRICE) + ".00", "currency": "RUB"},
                        "confirmation": {"type": "redirect", "return_url": f"https://t.me/{context.bot.username}"},
                        "capture": True,
                        "description": f"Подписка на Софию — пользователь {user_id}",
            ...
            if pay_url:
                ...
                await query.message.reply_text(
                    f"Нажмите кнопку для оплаты {SUBSCRIPTION_PRICE} руб:\n"
            ...
            await query.answer("Ошибка оплаты, попробуйте позже", show_alert=True)
    
    elif query.data == "sub_info":
        text = (
            "✨ *Что входит в подписку Софии:*\n\n"
            "📅 Планирование событий и задач\n"
            "🎯 Отслеживание целей\n"
            "📓 Дневник, покупки, финансы\n"
            "📝 Заметки\n"
            "❤️ Здоровье: вода, цикл, стресс\n"
            "🎙 Голосовые сообщения\n"
            "🎨 Генерация изображений\n"
            "🔍 Поиск в интернете\n"
            "⏳ Таймеры обратного отсчёта\n"
            "🌅 Утренний и вечерний отчёты\n\n"
            f"Всё это за {SUBSCRIPTION_PRICE} руб/месяц 🌸"
        )
...
async def cmd_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    info = await get_sub_info(user_id)
    if info["status"] == "free":
        await update.message.reply_text("✨ У вас активирован бесплатный доступ к Софии! 🌸")
        return
    if info["status"] == "active":
        await update.message.reply_text(f"✅ Подписка активна ещё {info['days_left']} дн. 🌸")
        return
    trial_text = ""
    if info["status"] == "trial":
        trial_text = f"⏳ Пробный период: осталось {info.get('days_left',0)} дн.\n\n"
    elif info["status"] == "expired":
        trial_text = "⚠️ Пробный период завершён.\n\n"
    text = f"{trial_text}💫 *Подписка на Софию* — {SUBSCRIPTION_PRICE} руб/мес\n\nВыберите способ оплаты:"
    keyboard = [
        [InlineKeyboardButton("💳 Карта / СБП (ЮKassa)", callback_data="pay_yukassa")],
        [InlineKeyboardButton(f"⭐ Telegram Stars ({STARS_PRICE} Stars)", callback_data="pay_stars")],
        [InlineKeyboardButton("ℹ️ Что входит в подписку", callback_data="sub_info")],
    ]
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    info = await get_sub_info(user_id)
    msgs = {
        "trial": f"⏳ Пробный период — осталось *{info.get('days_left',0)} дней*",
        "active": f"✅ Подписка активна — ещё *{info.get('days_left',0)} дней*",
        "free": f"🎁 Бесплатный доступ{' — ' + info.get('reason','') if info.get('reason') else ''}",
        "expired": "❌ Подписка истекла. Введите /subscribe для продления",
    }
    await update.message.reply_text(msgs.get(info["status"], "?"), parse_mode="Markdown")

async def handle_pre_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Подтверждаем Stars платёж"""
    await update.pre_checkout_query.answer(ok=True)

async def handle_successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """После успешной оплаты через Stars"""
    payment = update.message.successful_payment
    user_id = update.effective_user.id
    if payment.currency == "XTR":
        await activate_subscription(user_id, months=1, method="stars")
        await update.message.reply_text(
            "🎉 Оплата прошла успешно!\n\n"
            "✅ Подписка на Софию активирована на 30 дней.\n"
            "Приятного использования! 🌸"
        )
        await notify_admin(context, update.effective_user.full_name, 
                          update.effective_user.username or "",
                          "⭐ Оплатил Stars", f"Подписка активирована на 30 дней")
...
[{{"title":"название","person":"имя или пустая строка","time":"ЧЧ:ММ или пустая строка","repeat_type":"once/weekly/monthly_day","date":"ДД.ММ.ГГГГ если конкретная дата","weekdays":[0-6] если еженедельно,"month_day":число если каждое N-е число}}]"""
    ...
    @admin_app.get("/admin/subscriptions")
    async def get_subscriptions():
        ...
        try:
            ...
            total_paid = sum(r['total_paid'] or 0 for r in rows)
            active = [r for r in rows if r['status']=='active' and r['paid_until'] and r['paid_until']>now]
            trial = [r for r in rows if r['status']=='trial' and r['trial_end'] and r['trial_end']>now]
            ...
            return {
                "stats": {
                    ...
                    "trial": len(trial),
                    "free": len(free),
                    "expired": len(rows) - len(active) - len(trial) - len(free),
                    "mrr": len(active) * SUBSCRIPTION_PRICE,
                ...
                "subscriptions": [{
                    ...
                    "status": "free" if r['is_free'] else r['status'],
                    "paid_until": str(r['paid_until'])[:10] if r['paid_until'] else None,
                    "trial_end": str(r['trial_end'])[:10] if r['trial_end'] else None,
                    "total_paid": r['total_paid'] or 0,
                    "last_payment": str(r['last_payment'])[:10] if r['last_payment'] else None,
                    "is_free": r['is_free'],
                    "free_reason": r['free_reason'] or "",
                } for r in rows]
            }
        finally:
            await conn.close()

    @admin_app.post("/admin/subscription/free/{user_id}")
    async def give_free_access(user_id: int, body: dict = {}):
        reason = body.get("reason", "Администратор")
        await set_free_access(user_id, reason)
        ...
        try:
            ...
            import os
            bot_inst = Bot(token=os.getenv("BOT_TOKEN",""))
            await bot_inst.send_message(
                chat_id=user_id,
                text=f"✨ {name}, вам активирован бесплатный доступ к Софии! Приятного пользования 🌸"
            )
        except Exception as e:
            logging.error(f"notify free access: {e}")
        return {"ok": True}

    @admin_app.post("/admin/subscription/revoke/{user_id}")
    async def revoke_access(user_id: int):
        await revoke_free_access(user_id)
        return {"ok": True}

    @admin_app.post("/admin/subscription/activate/{user_id}")
    async def manually_activate(user_id: int, body: dict = {}):
        months = body.get("months", 1)
        await activate_subscription(user_id, months=months, method="manual")
        return {"ok": True}
