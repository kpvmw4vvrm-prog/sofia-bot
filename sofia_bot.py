...
[{{"title":"название","person":"имя или пустая строка","time":"ЧЧ:ММ или пустая строка","repeat_type":"once/weekly/monthly_day","date":"ДД.ММ.ГГГГ если конкретная дата","weekdays":[0-6] если еженедельно,"month_day":число если каждое N-е число}}]"""
    ...
    async def get_user_settings(user_id: int):
        ...
        try:
            ...
            if not row: return {}
            # Читаем доп. настройки из user_memory
            ...
            return {
                "lang": row["language"],
                "style": row["comm_style"],
                "morning": row["morning_plan"],
                "evening": row["evening_news"],
                "water_notif": row["water_reminders"],
                "water": row["water_reminders"],
                "events": mem.get("events", False),
                "goals_notif": mem.get("goals_notif", False),
            }
        finally:
            await conn.close()
    ...
    async def save_user_settings(body: dict):
        ...
        try:
            ...
            # Основные настройки — в таблицу users
            mapping = {
                "lang": "language",
                "style": "comm_style",
                "morning": "morning_plan",
                "evening": "evening_news",
                "water_notif": "water_reminders",
                "water": "water_reminders",
            }
            db_key = mapping.get(key)
            if db_key:
                await conn.execute(f"UPDATE users SET {db_key}=$1 WHERE user_id=$2", val, uid)
            # Дополнительные — в user_memory
            elif key in ("events", "goals_notif"):
                ex = await conn.fetchrow("SELECT id FROM user_memory WHERE user_id=$1 AND key=$2", uid, key)
                if ex:
                    await conn.execute("UPDATE user_memory SET value=$1, updated_at=NOW() WHERE user_id=$2 AND key=$3", str(val), uid, key)
                else:
                    await conn.execute("INSERT INTO user_memory (user_id, key, value) VALUES ($1,$2,$3)", uid, key, str(val))
            return {"ok": True}
        finally:
            await conn.close()
