...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing and existing.get("onboarded"):
        ...
        name = existing.get("name") or user_name  # берём имя из Telegram если нет в БД
        if lang == "ru":
    ...
    # Новый пользователь или после сброса — онбординг с нуля
    ...
    async def api_reset(user_id: int, auth=Depends(check_token)):
        async with db_pool.acquire() as conn:
            for tbl in ["history","reminders","notes","habits","habit_logs","finances",
                        ...
                        "goals","weight_logs","nutrition_profile","food_logs","birthdays",
                        "mood_logs","water_logs","subscriptions"]:
                try:
                    await conn.execute(f"DELETE FROM {tbl} WHERE user_id=$1", user_id)
                except: pass
            await conn.execute(
                "UPDATE users SET onboarded=FALSE, name=NULL, language='ru' WHERE user_id=$1",
                user_id
            )
        # Отправляем пользователю приветствие — онбординг начнётся при /start
        try:
            from telegram import Bot as _Bot
            import os as _os
            _bot = _Bot(token=_os.getenv("BOT_TOKEN",""))
            await _bot.send_message(
                chat_id=user_id,
                text="🌸 Привет! Ваш аккаунт был сброшен администратором.\n\nНапишите /start чтобы начать заново."
            )
        except Exception as e:
            logging.error(f"reset notify: {e}")
        return {"ok": True, "message": "Аккаунт сброшен, пользователь уведомлён"}
