...
def get_submenu(section, lang="ru", user_id=None):
    ...
    if section == "planning":
        keyboard = [
            [InlineKeyboardButton("📆 Календарь", web_app=WebAppInfo(url=f"{BASE}?tab=planning&view=cal{uid_param}")),
             InlineKeyboardButton("✅ Задачи", web_app=WebAppInfo(url=f"{BASE}?tab=planning&view=tasks{uid_param}"))],
            [InlineKeyboardButton("⏰ Напоминания", web_app=WebAppInfo(url=f"{BASE}?tab=planning&view=rems{uid_param}")),
             InlineKeyboardButton("⏳ Таймер", web_app=WebAppInfo(url=f"{BASE}?tab=planning&view=timer{uid_param}"))],
            [back],
        ]
        title = "📅 Планирование" if ru else "📅 Planning"
    elif section == "goals":
        keyboard = [
            [InlineKeyboardButton("🔥 Активные цели", web_app=WebAppInfo(url=f"{BASE}?tab=goals&view=active{uid_param}")),
             InlineKeyboardButton("✅ Завершённые", web_app=WebAppInfo(url=f"{BASE}?tab=goals&view=done{uid_param}"))],
            [InlineKeyboardButton("📊 Статистика", web_app=WebAppInfo(url=f"{BASE}?tab=goals&view=stats{uid_param}"))],
            [back],
        ]
        ...
        keyboard = [
            [InlineKeyboardButton("💧 Вода", web_app=WebAppInfo(url=f"{BASE}?tab=health&view=water{uid_param}")),
             InlineKeyboardButton("💊 Таблетки", web_app=WebAppInfo(url=f"{BASE}?tab=health&view=meds{uid_param}"))],
            [InlineKeyboardButton("🩸 Цикл", web_app=WebAppInfo(url=f"{BASE}?tab=health&view=cycle{uid_param}")),
             InlineKeyboardButton("😰 Стресс", web_app=WebAppInfo(url=f"{BASE}?tab=health&view=stress{uid_param}"))],
            [back],
        ]
        title = "❤️ Здоровье" if ru else "❤️ Health"
    elif section == "about":
        keyboard = [
            [InlineKeyboardButton("✨ Открыть", web_app=WebAppInfo(url=f"{BASE}?tab=about{uid_param}"))],
            [back],
        ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Настройки
        ...
        result = get_submenu(section, lang, user_id=user_id)
        ...
        if is_reminder_request(user_text):
            ...
            if rel_value is not None:
                ...
                if hour is not None:
                    ...
                    if conflict:
                        dupe_msg = f"Не переживайте, я помню! ❤️\n\nВ {time_str} у вас уже запланировано: «{conflict}». Всё записано, напомню вовремя 🌸" if lang=="ru" else f"Don't worry, I remember! ❤️\n\nAt {time_str}: «{conflict}» 🌸"
                        await update.message.reply_text(dupe_msg)
                        await notify_admin(context, user_name, username, user_text, dupe_msg)
                        return
...
[{{"title":"название","person":"имя или пустая строка","time":"ЧЧ:ММ или пустая строка","repeat_type":"once/weekly/monthly_day","date":"ДД.ММ.ГГГГ если конкретная дата","weekdays":[0-6] если еженедельно,"month_day":число если каждое N-е число}}]"""
    ...
    @admin_app.delete("/app/reminders/{reminder_id}")
    async def delete_reminder_app(reminder_id: int, user_id: int):
        conn = await db()
        try:
            await conn.execute("DELETE FROM reminders WHERE id=$1 AND user_id=$2", reminder_id, user_id)
            return {"ok": True}
        finally:
            await conn.close()

    @admin_app.get("/app/notes/{user_id}")
    async def get_notes_app(user_id: int):
        conn = await db()
        try:
            try: await conn.execute("ALTER TABLE notes ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP")
            except: pass
            rows = await conn.fetch("SELECT id, text, created_at, updated_at FROM notes WHERE user_id=$1 ORDER BY COALESCE(updated_at,created_at) DESC", user_id)
            return [{"id":r["id"],"text":r["text"],"created_at":str(r["created_at"]),"updated_at":str(r["updated_at"] or r["created_at"])} for r in rows]
        finally:
            await conn.close()

    @admin_app.post("/app/notes")
    async def add_note_app(body: dict):
        conn = await db()
        try:
            try: await conn.execute("ALTER TABLE notes ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP")
            except: pass
            row = await conn.fetchrow("INSERT INTO notes (user_id, text) VALUES ($1,$2) RETURNING id, text, created_at", body["user_id"], body["text"])
            return {"id":row["id"],"text":row["text"],"created_at":str(row["created_at"]),"updated_at":str(row["created_at"])}
        finally:
            await conn.close()

    @admin_app.patch("/app/notes/{note_id}")
    async def update_note_app(note_id: int, body: dict):
        conn = await db()
        try:
            try: await conn.execute("ALTER TABLE notes ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP")
            except: pass
            await conn.execute("UPDATE notes SET text=$1, updated_at=NOW() WHERE id=$2 AND user_id=$3", body["text"], note_id, body["user_id"])
            return {"ok": True}
        finally:
            await conn.close()

    @admin_app.delete("/app/notes/{note_id}")
    async def delete_note_app(note_id: int, user_id: int):
        conn = await db()
        try:
    ...
    async def get_user_settings(user_id: int):
        ...
        try:
            ...
            if not row: return {}
            mem_rows = await conn.fetch("SELECT key, value FROM user_memory WHERE user_id=$1 AND key IN ('events','goals_notif')", user_id)
            mem = {r["key"]: r["value"] == "True" for r in mem_rows}
            return {
                "lang": row["language"], "style": row["comm_style"],
                "morning": row["morning_plan"], "evening": row["evening_news"],
                "water_notif": row["water_reminders"], "water": row["water_reminders"],
                "events": mem.get("events", False), "goals_notif": mem.get("goals_notif", False),
            }
        finally:
            await conn.close()
    ...
    async def save_user_settings(body: dict):
        conn = await db()
        try:
            key = body.get("key"); val = body.get("value"); uid = body.get("user_id")
            mapping = {"lang":"language","style":"comm_style","morning":"morning_plan","evening":"evening_news","water_notif":"water_reminders","water":"water_reminders"}
            db_key = mapping.get(key)
            if db_key:
                await conn.execute(f"UPDATE users SET {db_key}=$1 WHERE user_id=$2", val, uid)
            elif key in ("events","goals_notif"):
                ex = await conn.fetchrow("SELECT id FROM user_memory WHERE user_id=$1 AND key=$2", uid, key)
                if ex: await conn.execute("UPDATE user_memory SET value=$1, updated_at=NOW() WHERE user_id=$2 AND key=$3", str(val), uid, key)
                else: await conn.execute("INSERT INTO user_memory (user_id, key, value) VALUES ($1,$2,$3)", uid, key, str(val))
            return {"ok": True}
        finally:
            await conn.close()
