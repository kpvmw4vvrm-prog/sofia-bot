...
async def is_subscribed(user_id: int) -> bool:
    ...
    async with db_pool.acquire() as conn:
        ...
        if row['is_free']:
            return True
        now = datetime.now()
        if row['status'] == 'trial' and row['trial_end'] and row['trial_end'] > now:
            return True
        if row['status'] == 'active' and row['paid_until'] and row['paid_until'] > now:
            return True
        return False
...
async def get_sub_info(user_id: int) -> dict:
    ...
    async with db_pool.acquire() as conn:
        ...
        if row['status'] == 'trial' and row['trial_end']:
            ...
            return {"status": "trial", "days_left": max(0, days)}
        if row['status'] == 'active' and row['paid_until']:
            days = (row['paid_until'] - now).days
            return {"status": "active", "days_left": max(0, days)}
