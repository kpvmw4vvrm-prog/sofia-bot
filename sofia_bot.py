...
ЧТО УМЕЕШЬ: планирование, напоминания, поддержка, нутрициология, цели, рецепты, погода, поиск, любые вопросы.
...
async def send_evening_news(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"Составь тёплый вечерний итог дня для {user['name']}. Сегодня {date_str}.\n\n{data_block}\n\nКак личное колесо баланса — что хорошо получилось, что улучшить завтра, мотивация. 3-4 абзаца, без звёздочек."
        resp = ai_client.chat.completions.create(model="gpt-4o-mini", messages=[{"role":"user","content":prompt}], max_tokens=600, temperature=0.8)
        await context.bot.send_message(chat_id=user_id, text=resp.choices[0].message.content)
    except Exception as e:
...
async def ask_comm_style_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lang == "en":
        ...
        keyboard = [["👭 Подружка — тепло, неформально, на ты"],["🎯 Наставник — мотивирующий, на вы"],["💼 Профессионал — чётко и по делу"]]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Дневник
        ...
        if events:
            ...
            text = "Планер пуст.\n\nФиксированные регулярные занятия — тренировки, курсы, встречи.\n\nНапример: каждую среду в 17:00 тренировка"
        keyboard = [
...
async def process_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str):
    ...
    if user_text.strip().lower() in ["да","yes","добавь","запиши"] and context.user_data.get("pending_planner_text"):
        ...
        event_title = re.sub(r"(каждый|каждую|каждое|всегда|регулярно|по\s+\w+|у меня)\s*","", raw_title, flags=re.IGNORECASE).strip()
    ...
    if context.user_data.get("waiting_planner"):
        ...
        if day_num is not None and h is not None:
            time_str = str(h).zfill(2) + ":" + str(m).zfill(2)
            parts = user_text.split(time_str)
            event_title = parts[-1].strip().lstrip("-— ") if len(parts) > 1 else user_text
            async with db_pool.acquire() as conn:
                await conn.execute("INSERT INTO planner (user_id, day_of_week, time_str, title) VALUES ($1, $2, $3, $4)", user_id, day_num, time_str, event_title)
            reply = f"Записала в планер! Каждый {DAYS_RU_NAMES[day_num]} в {time_str} — {event_title}"
            await update.message.reply_text(reply)
            await notify_admin(context, user_name, username, user_text, reply)
        else:
    ...
    try:
        ...
        # Предлагаем планер для регулярных занятий
        regular_kw = ["всегда","каждый ","каждую ","каждое ","регулярно","постоянно","по понедельникам","по вторникам","по средам","по четвергам","по пятницам","по субботам","по воскресеньям"]
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users_list:
        ...
        lines.append(f"ID: {u['user_id']} | {u['name'] or '?'} @{u['username'] or '?'} | {date} | {u['language']} | {u['comm_style']}")
...
async def handle_photo(update, context):
    ...
    try:
        ...
        if has_nutr and not caption:
            context.user_data["pending_food_photo"] = image_data
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("Да, посчитай калории", callback_data="count_food_calories"), InlineKeyboardButton("Нет, просто опиши", callback_data="just_describe_photo")]])
            await update.message.reply_text("Это фото еды? Посчитать калории?", reply_markup=kb)
            return
        reply = await analyze_image(image_data, caption, lang)
        await update.message.reply_text(reply)
        await notify_admin(context, user_name, username, "[Фото]" + (" " + caption if caption else ""), reply)
...
# === АДМИН ===
async def admin_check(update) -> bool:
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("Нет доступа.")
        return False
    return True
...
async def stats(update, context):
    ...
    async with db_pool.acquire() as conn:
        total = await conn.fetchval("SELECT COUNT(*) FROM users WHERE onboarded = TRUE")
        today_active = await conn.fetchval("SELECT COUNT(DISTINCT user_id) FROM history WHERE created_at >= NOW() - INTERVAL 1 day".replace("1 day","'1 day'"))
        week_active = await conn.fetchval("SELECT COUNT(DISTINCT user_id) FROM history WHERE created_at >= NOW() - INTERVAL '7 days'")
        total_msg = await conn.fetchval("SELECT COUNT(*) FROM history WHERE role = 'user'")
    msg = f"Статистика Софии\n\nПользователей: {total}\nАктивны сегодня: {today_active}\nЗа 7 дней: {week_active}\nВсего сообщений: {total_msg}\n\nАдмин команды:\n/users — список\n/announce текст — рассылка\n/reset ID — сброс\n/ban ID — блок\n/unban ID — разблок\n/msg ID текст — написать"
    await update.message.reply_text(msg)
...
async def msg_user(update, context):
    if not await admin_check(update): return
    if not context.args or len(context.args) < 2:
        await update.message.reply_text("Использование: /msg USER_ID Текст")
        return
    try:
        ...
        text = " ".join(context.args[1:])
        await context.bot.send_message(chat_id=target_id, text=text)
        await update.message.reply_text(f"Сообщение отправлено {target_id}.")
    except Exception as e:
        await update.message.reply_text(f"Ошибка: {e}")

async def reset_user(update, context):
    if not await admin_check(update): return
    if not context.args:
        await update.message.reply_text("Использование: /reset USER_ID")
        return
...
async def post_init(application):
    ...
    application.job_queue.run_daily(check_cycle_reminders, time=_time(hour=9, minute=0), name="cycle_check")
    application.job_queue.run_daily(check_goal_reminders, time=_time(hour=12, minute=0), name="goal_check")
    application.job_queue.run_daily(check_birthday_reminders, time=_time(hour=10, minute=0), name="birthday_check")
    application.job_queue.run_daily(send_weekly_habit_report, time=_time(hour=20, minute=0), name="weekly_habits")
...
if __name__ == "__main__":
    ...
    # Онбординг — start = ТОЛЬКО приветствие, НЕ меню!
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start)],
        states={
            ASK_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_name)],
    ...
    # /menu — открывает меню (отдельная команда)
    ...
    print(f"Sofia v6.1 started! Admin={ADMIN_ID}")
    app.run_polling()
